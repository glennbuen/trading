#!/usr/bin/env python3
"""
OKX Bollinger-Bands-Only Day Trading Bot

Deliberately minimal: Bollinger Bands are the ONLY indicator driving
entries, exits, AND risk — no volume filter, no trend filter (EMA/ADX), no
ATR. Stop and target distances are derived purely from the bands
themselves (band width), per the "bollinger bands only" brief. The only
non-indicator additions are execution/risk mechanics shared by every bot
in this repo (fees, slippage, circuit breakers, forced day-end flatten) —
those are order-management plumbing, not a second signal source.

Usage:
    python okx_bollinger_bot.py                  # paper mode (default, no real orders)
    python okx_bollinger_bot.py --live           # live mode (requires explicit flag)
    python okx_bollinger_bot.py --backtest 3000  # backtest last 3000 candles (paginated)

Env vars required for --live / --demo:
    OKX_API_KEY, OKX_API_SECRET, OKX_PASSPHRASE

Core signal logic (classic Bollinger mean-reversion, not a squeeze/breakout
read — bands here mark "stretched," not "about to run"):
    - Bands: SMA(20) +/- 2 x rolling std(20).
    - Long: prior bar touched/pierced the lower band, current close has
      reclaimed back above it ("touch and reclaim").
    - Short: mirrored at the upper band ("touch and reject"). Requires
      --allow-shorts (spot can't short).
    - Exit: target = the middle band (SMA) — the classic mean-reversion
      target. Stop = entry extended by `stop_band_frac` x the CURRENT band
      width, in the adverse direction — a purely band-derived stop, no ATR.
    - Forced flatten if the position is still open when the UTC day rolls
      over (day-trading discipline, same as the other day-scoped bots
      here).
...
CONFIG = {
    ...
    # Bollinger Bands — the only indicator in this bot
    "bb_len": 20,
    "bb_std": 2.0,
    "stop_band_frac": 0.5,      # stop = entry +/- 0.5x current band width
    ...
    # Circuit breakers (execution/risk plumbing, not a second indicator)
    ...
    # Ops
    "poll_seconds": 60,
    "state_file": "bollinger_state.json",
    "log_file": "bollinger.log",
}
...
log = logging.getLogger("okx-bollinger-bot")
...
def compute_signals(df: pd.DataFrame, cfg: dict) -> pd.DataFrame:
    ...
    d["sma"] = d["close"].rolling(cfg["bb_len"]).mean()
    d["std"] = d["close"].rolling(cfg["bb_len"]).std()
    d["upper"] = d["sma"] + cfg["bb_std"] * d["std"]
    d["lower"] = d["sma"] - cfg["bb_std"] * d["std"]
    d["band_width"] = d["upper"] - d["lower"]

    touched_lower = d["low"].shift(1) <= d["lower"]
    reclaim_lower = d["close"] > d["lower"]
    long_trigger = touched_lower & reclaim_lower

    touched_upper = d["high"].shift(1) >= d["upper"]
    reject_upper = d["close"] < d["upper"]
    short_trigger = touched_upper & reject_upper

    d["long_signal"] = long_trigger
    d["short_signal"] = short_trigger
...
def backtest(d: pd.DataFrame, cfg: dict):
    ...
    for i in range(warmup, len(d)):
        ...
        if position is None and (i - last_stop_bar) >= cfg["cooldown_bars"] and i > halt_until_bar:
            ...
            if side and pd.notna(row["band_width"]):
                slip = 1 + (slippage_pct / 100) * (1 if side == "long" else -1)
                entry = row["close"] * slip
                stop_dist = cfg["stop_band_frac"] * row["band_width"]
...
def run_loop(cfg, live: bool, demo: bool = False):
    ...
    log.info(f"Starting in {mode} mode — {cfg['symbol']} {cfg['timeframe']} Bollinger-only")
    ...
    while True:
        try:
            ...
            log.info(
                ...
                f"bands[{row['lower']:.2f} | {row['sma']:.2f} | {row['upper']:.2f}] "
...
def main():
    ...
    ap.add_argument("--bb-len", type=int, help="override Bollinger Band period (default 20)")
    ap.add_argument("--bb-std", type=float, help="override Bollinger Band std-dev multiple (default 2.0)")
    ap.add_argument("--stop-band-frac", type=float, help="override stop distance as a fraction of band width (default 0.5)")
    args = ap.parse_args()
    ...
    if args.bb_len:
        cfg["bb_len"] = args.bb_len
    if args.bb_std:
        cfg["bb_std"] = args.bb_std
    if args.stop_band_frac:
        cfg["stop_band_frac"] = args.stop_band_frac
    ...
    if args.backtest:
        ...
        d = compute_signals(df, cfg)
        res = backtest(d, cfg)
        print(f"\n=== BACKTEST: {cfg['symbol']} {cfg['timeframe']} Bollinger-only "
