...
Regime detection, BTC-context monitoring, the full trade journal (beyond the backtest engine's own `rejected_signals` list — already a real instance of spec §31's "record rejected signals too"), dashboard, and the last remaining strategy (liquidity sweep reversal). See the repo README's roadmap table for phase order.
